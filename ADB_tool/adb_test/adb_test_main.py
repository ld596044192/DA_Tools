...
# 统一修改版本号
adb_version = 'V1.0.1.15'
adb_version_code = 10115
# 统一修改frame的宽高
width = 367
height = 405
# 统一按钮宽度
...
class ADB_Test(object):
    def adb_root_form(s,root):
        ...
        s.root.title('ADB测试工具' + adb_version + ' Windows版')
    ...
    def adb_close(s):
        def frame_page_clear():
            # 下面做了很多异常处理和循环检测是确保退出该功能后不会有残留frame和页面
            for i in range(15):
                try:
                    s.main_frame.place_forget()
        ...
        def t_adb_close():
            ...
            if tkinter.messagebox.askyesno(title='退出提醒',message='确定要退出ADB测试工具？\n点击"确定"这将返回到主界面'):
                ...
                s.close_str.set('正在等待所有窗口响应...')
                # 关闭frame
                s.close_str.set('正在关闭ADB测试工具所有frame...')
                frame_page_clear()  # clean，防止组件显示残留
                ...
                s.moving_radio_button1.place_forget()
                s.moving_radio_button2.place_forget()
                s.adb_test_close.place_forget()
                s.close_label.place_forget()

                with open(function_status,'w') as fp:
                    fp.write('close')
                fp.close()
                frame_page_clear()  # 最终clean，杜绝组件显示残留
                print('ADB测试工具已完全退出！')
    ...
    def main_menu_bar(s):
        # 切换窗口（选择模式）
        s.main_frame = tkinter.Frame(s.root,width=380,height=20)
        ...
        s.screen_menu = tkinter.Menubutton(s.main_frame, text='截图录屏')
        s.screen_menu1 = tkinter.Menubutton(s.main_frame, text='截图录屏')
        s.screen_menu1.config(state='disable')

        s.install_menu = tkinter.Menubutton(s.main_frame, text='安装卸载')
        s.install_menu1 = tkinter.Menubutton(s.main_frame, text='安装卸载')
        s.install_menu1.config(state='disable')

        s.linux_menu = tkinter.Menubutton(s.main_frame, text='Linux模式')
        s.linux_menu1 = tkinter.Menubutton(s.main_frame, text='Linux模式')
        s.linux_menu1.config(state='disable')

        s.customize_menu = tkinter.Menubutton(s.main_frame, text='自定义模式')
        ...
        # 退出ADB测试工具按钮
        s.adb_test_close = tkinter.Button(s.root,text='退出ADB测试工具',width=width_button)
        s.adb_test_close_disable = tkinter.Button(s.root,text='退出ADB测试工具',width=width_button)
        ...
        def t_md5_size_close():
            ...
            while True:
                ...
                if md5_size_page_state.strip() == '0':
                    ...
                    s.md5_size_button.place(x=200, y=20)
                    try:
                        public.stop_thread(t_md5_size)  # 关闭后终止所有计算md5和大小的行为
                    except ValueError:
                        pass
                    s.adb_test_close_disable.place_forget()
                    break
